...
class PDB:
        ...
        if residue in self.__res_list[0]:
            ...
            if missing_ids[1]:
                for i in missing_ids[1]:
                    if residue.get_id()[1] > i:
                        res_id -= 1
            res_id += residue.get_id()[1] - b_shift + len(self.__res_list[0])

        return res_id

    def alt_cor_shift(self, residue):
        a_shift = self.__res_list[0][0].get_id()[1]
        b_shift = self.__res_list[1][0].get_id()[1]
        ...
        if residue in self.__res_list[0]:
            ...
            if missing_ids[1]:
                for i in missing_ids[1]:
                    if residue.get_id()[1] > i:
                        res_id -= 1
            res_id += residue.get_id()[1] - b_shift
    ...
    def ICs(self):
        ...
        ics = sorted(list(set((map(self.correct_shift, flat_list)))))
        return ics
    ...
    def pairs(self):
        ...
        pairs = self.__reference()
        c_map = np.asarray([list(map(self.alt_cor_shift, pair)) for pair in pairs])
        return c_map
    ...
    def B_ICs(self):
        ...
        b_ics = [i[1] for i in pairs]
        b_ics = sorted(list(set(map(self.correct_shift, b_ics))))
        b_ics = list(map(lambda x: x - len(self.__res_list[0]), b_ics))
        return b_ics
